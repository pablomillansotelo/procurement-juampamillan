...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = new URL(`${PROCUREMENT_API_URL}/v1/receipts`);
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.message || 'Error al obtener recepciones' },
    ...
    console.error('Error en GET /api/procurement/v1/receipts:', error);
    return NextResponse.json(
      { error: error.message || 'Error al obtener recepciones' },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${PROCUREMENT_API_URL}/v1/receipts`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-API-Key': PROCUREMENT_API_KEY,
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      return NextResponse.json(
        { error: errorData.message || 'Error al crear recepción' },
        { status: response.status }
      );
    }
    ...
    console.error('Error en POST /api/procurement/v1/receipts:', error);
    return NextResponse.json(
      { error: error.message || 'Error al crear recepción' },
      { status: 500 }
    );
  }
}
